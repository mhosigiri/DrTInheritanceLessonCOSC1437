/*
Using Dr_T's Color in CPP as a Header //filename: Color_in_CPP.h in my solution
folder
*/
...
char showMenu(char &selectedOption) {
  ...
  cout << "D = Draw Diamond" << endl;
  cout << "O = Draw Oval" << endl;
  cout << "P = Draw Trapezoid" << endl;
  cout << "Y = Draw Pyramid" << endl;
  cout << "E = Exit" << endl;
...
void drawRect(RectangleShape &Rect) {
  ...
  cout << "with width = " << Rect.getWidth()
       << " and length = " << Rect.getLength();
...
void drawTri(TriangleShape &Tri) {
  ...
  cout << "with base = " << Tri.getBase() << " and sideA = " << Tri.getSideA()
       << " and sideC = " << Tri.getSideC();
...
void drawCirc(CircleShape &Circle) {
  ...
  if (outfile.is_open()) {
    ...
    for (int y = -Circle.getRadius(); y <= Circle.getRadius(); y++) {
      for (int x = -console_ratio * Circle.getRadius();
           x <= console_ratio * Circle.getRadius(); x++) {
...
void drawDiamond(DiamondShape &Diamond) {
  int j;
  int i;
  ...
  changeColor(32);
  cout << "\n\n**********Diamond Program***********" << endl;
  changeColor(7);

  cout << fixed << setprecision(3);
  Diamond.setName("Diamond_1");

  cout << "Kindly provide me a base for the Diamond. ";
  userInput = validateDouble(userInput);
  Diamond.setBase(userInput);
  cout << "Kindly provide me a height for the Diamond. ";
  userInput = validateDouble(userInput);
  Diamond.setHeight(userInput);
  cout << "Kindly provide me a side for the Diamond. ";
  userInput = validateDouble(userInput);
  Diamond.setSideA(userInput);

  // Print the Perimeter of the object.
  cout << "\nThe Perimeter for: " << Diamond.getName() << endl;
  cout << "with side = " << Diamond.getSideA();
  cout << " is " << Diamond.getPerimeter();

  // Print the area of the object.
  cout << "\n\nThe Area for: " << Diamond.getName() << endl;
  cout << "with side = " << Diamond.getSideA()
       << " and base = " << Diamond.getBase()
       << " and height = " << Diamond.getHeight();
  cout << " is " << Diamond.getArea() << endl;

  Diamond.drawShape();              // draw the Square on screen with *
  outfile.open("DiamondShape.txt"); // draw the shape in a file
  if (outfile.is_open()) {
    outfile << "\nDrawing for: " << Diamond.getName() << endl;

      cout << "\nDrawing for: " << Diamond.getName() << endl;
        for ( i = 1; i <= Diamond.getHeight(); i++) {
            for ( j = i; j < Diamond.getHeight(); j++) {
              cout << " ";
            }
            for ( j = 1; j <= Diamond.getHeight(); j++) {
              cout << "*";
            }
            cout << endl;
          }
          for( i = 1; i <= Diamond.getHeight() - 1; i++){
            for( j = 1; j <= i; j++){
              cout << " ";
            }
            cout << endl;
  ...
  //system("DiamondShape.txt"); // open the output file
